...
resource "aws_kms_key" "soydaner-bucket_kms_key" {
  description             = "Encrypt the state bucket objects"
  deletion_window_in_days = 10
}

resource "aws_s3_bucket" "soydaner-bucket" {
  bucket = "dm-soydaner-tmp-bucket"
}

resource "aws_s3_bucket_versioning" "soydaner-bucket_versioning" {
  bucket = aws_s3_bucket.soydaner-bucket.id
  versioning_configuration {
    status = "Enabled"
  }
}
resource "aws_s3_bucket_server_side_encryption_configuration" "soydaner-bucket_encryption" {
  bucket = aws_s3_bucket.soydaner-bucket.id

  rule {
    apply_server_side_encryption_by_default {
      kms_master_key_id = aws_kms_key.soydaner-bucket_kms_key.arn
...
resource "aws_s3_bucket_public_access_block" "soydaner-bucket_acl" {
  bucket = aws_s3_bucket.soydaner-bucket.id
...
resource "aws_secretsmanager_secret" "soydaner-secret" {
  name        = "dm-soydaner-secret"
  description = "soydaner-secret"
}
resource "aws_secretsmanager_secret_version" "soydaner-secret_version" {
  secret_id = aws_secretsmanager_secret.soydaner-secret.id
...
#  env_name              = var.env_name
#  region                = var.region
#  account_id            = var.account_type == "prod" ? var.account_id_prod : var.account_id_syd
#  private_subnet_one_id = module.vpcmodule.private_subnets_output[0]
#  private_subnet_two_id = module.vpcmodule.private_subnets_output[1]
